...
describes what users get. Pick a format with `-o`, or let ibge choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ibge <command> -o table   # aligned columns for reading
ibge <command> -o jsonl   # one JSON object per line, for piping
ibge <command> -o json    # a single JSON array
ibge <command> -o csv     # spreadsheet friendly
ibge <command> -o tsv     # tab-separated
ibge <command> -o url     # just the URL column
ibge <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ibge <command> --fields id,title,url
```
...
```bash
ibge <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ibge <command>            # a table, because this is a terminal
ibge <command> | wc -l    # JSONL, because this is a pipe
```
