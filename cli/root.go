// Package cli assembles the ibge command tree from the ibge
...
// NewApp assembles the kit application from the ibge domain. The
...
//
// To add a command, declare it in ibge/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := ibge.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(ibge.Domain{}).Register(app)
